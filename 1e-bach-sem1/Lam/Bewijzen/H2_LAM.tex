...
\begin{proof}(vii)
    ...
    \begin{align*}
        \lambda v &= 0 \\
        \lambda^{-1}\lambda v &= \lambda^{-1}0 = 0\\
        v &= 0
    \end{align*}
\end{proof}
...
\begin{proof}
    Gegeven: \(S\) is voortbrengend voor \(V\), dus \(\operatorname{span}(S) = V\).

    \(S = \{s_1, \dots, s_n\}\) bevat \(n\) elementen waarvan elke vector in \(V\) een lineaire combinatie van is, dus \(\forall v \in V\) kan \(v\) geschreven worden als \(\sum_{i}^{n}\mu_i s_i\),
    met \(\sum_{j}^{n} \mu_j s_j = 0 \implies \forall \mu_i \in K: \mu_i = 0 .\)

    Stel we hebben een lineair onafhankelijke deelverzameling \(U = \{u_1, \dots , u_{n+1}\}\) met \(n +1\) elementen die lineair onafhankelijk zijn.

    Dit wil zeggen dat \(\sum_{i}^{n+1} \gamma_i u_i = 0 \implies \forall \gamma_i \in K: \gamma_i = 0 . \)

    Doordat \(U\) een deelverzameling is van \(V\) geldt \(\forall u_i \in U: u_i = \sum_{j}^{n} \mu_{ij} s_j, \, i \in \{1,n\}, \, j \in \{1,n+1\}. \)

    We zoeken alle mogelijkheden zodat \(\sum_{i}^{n+1} \gamma_i u_i = 0\):

    \begin{align*}
            \sum_{i}^{n+1} \gamma_i u_i &= \sum_{i}^{n+1} \gamma_i \sum_{j}^{n} \mu_{ij} s_j\\
            &= \sum_{i}^{n+1} \sum_{j}^{n} \gamma_i\mu_{ij} s_j\\
            &= \sum_{j}^{n}\left(\sum_{i}^{n+1}  \gamma_i\mu_{ij} \right) s_j\\
            &=  0
    \end{align*}
   
    Dit resulteert in de oplossingen zoeken voor \(\sum_{i}^{n+1}  \gamma_i\mu_{ij} = 0 \, \forall j \in \{1,\dots,n\}\).
    Wat equivalent is aan een homogeen \(n \times n+1\) stelsel oplossen. Wat ons 1 vrijheidsgraad opleverd. 

    Bijgevolg is elke verzameling met \(n+1\) elementen lineair afhankelijk
...
\begin{proof}(i)
    Als \(V\) lineair afhankelijk is, pas gevolg 2.4.9 (ii) systematisch toe tot \(V\) ze lineair onafhankelijk is.
    Als \(V\) niet voortbrengend is, pas gevolg 2.4.9 (i) systematisch toe tot \(V\) ze voortbrengend is. 
\end{proof}

\begin{proof}(ii)
    Stel basis \(\mathcal{B}\) en een equivalente basis \(\mathcal{C}\).
...
\begin{proof}(i)
    ...
    Door dat dit geldt voor de basissen apart, gelt de lineair onafhankelijkheid ook voor de som van de basissen.
    De voortbrengendheid volgt uit dat \(\forall w_1 \in W_1: w_{1,i} = \sum_{i}^{n} \mu_i b_i \) en
     \(\forall w_2 \in W_2: w_{2,j} = \sum_{j}^{m} \mu_j b_j \).

    Uit \(W = W_1 + W_2\) volgt \(\forall w \in W: w_k = \sum_{i}^{n} \mu_i b_i + \sum_{j}^{m} \nu_j c_j\).

\end{proof}

\begin{proof}(ii)
    \(\dim(W)= \) het aantal elementen van de basis voor \(W\). 
...
\begin{proof}
    ...
    \(\mathcal{B}_1 \cap \mathcal{B}_2 = \emptyset \implies \operatorname{span} (\mathcal{B}_1) \cap \operatorname{span}(\mathcal{B}_2) = \{0\} \implies V_1 \cap V_2 = \{0\}\).
...
\begin{proof}
    \(W_1 + W_2  = V \implies W_1 \oplus \left(W_2 - W_1 \cap W_2\right) = V\)

    Nu gebruiken we de dimensiestelling 2.5.4 voor directie sommen. 
    \begin{align*}
        \dim(V) 
        &= \dim(W_1 \oplus \left(W_2 - W_1 \cap W_2\right))\\
        &= \dim(W_1) + \dim(W_2 - W_1 \cap W_2)\\
        &= \dim(W_1) + \dim(W_2) - \dim(W_1 \cap W_2)\\
    \end{align*}

    \textbf{Dit is nog fout!}
\end{proof}
...
\begin{proof}
    ...
    Stel nu \(W \cap U \neq \{0\}\) dan nemen we \(Y = U - \left(U \cap W\right)\),
    zodat:
    \begin{align*}
        W \cap Y &= W \cap \left(U - \left(U \cap W\right)\right)\\
        &=  W \cap U - W \cap \left(U \cap W\right)\\
        &=  W \cap U -  W\cap U\\
        &= 0
    \end{align*}

    Bijgevolg is opnieuw \(V = W \oplus Y\).
...
\begin{proof}
    ...
    Om te controlleren of deze ruimte een vectorruimte is, moeten we V1-V9 controlleren:
    ...
    \textbf{Te lang, doe ik andere keer.}
