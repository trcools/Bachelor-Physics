...
\secblock{H2 Electrostatics}
    ...
    E field capacitor:
    \(\bm E = \frac{\sigma}{\epsilon_0} \hat{\bm n}\); \(V = \frac{Qd}{A\epsilon_0}\)
    ...
    \(V_{\text{quad}}(\bm r)  = \frac{1}{4\pi\epsilon_0r^3}\hat{\bm r_i}\hat{\bm r_j}Q_{ij}\)

    \(Q_{ij} = \frac{1}{2}\sum_k (3r_{i}r_{j}-r^2_k\delta_{ij})q_k\)
...
Perfect conductor (inside):
\(\bm E = 0\);
\(\frac{\partial \bm B}{\partial t} = 0\)

Superconductor (inside):
\(\bm E = 0\); \(\bm B = 0\)
