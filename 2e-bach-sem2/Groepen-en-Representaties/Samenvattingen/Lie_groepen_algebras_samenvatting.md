...
## 5. Eén overzichtstabel om alles samen te vatten

| | $SO(2)$ | $U(1)$ | $SO(3)$ | $SU(2)$ |
|---|---|---|---|---|
| **Type matrices** | reëel $2\times2$, orthogonaal, $\det=1$ | $\|z\|=1 \subset \mathbb{C}$ | reëel $3\times3$, orthogonaal, $\det=1$ | complex $2\times2$, unitair, $\det=1$ |
| **Abels?** | ja | ja | nee | nee |
| **Parametrisatie** | $\theta\in[0,2\pi)$ | $\theta\in[0,2\pi)$ | as-hoek, Euler-hoeken, $R_z R_y R_x$ | $(\alpha,\beta)\in S^3$, of $e^{i\theta\sigma_{\vec n}}$ |
| **Topologie** | cirkel $S^1$ | cirkel $S^1$ | $\mathbb{RP}^3$ (niet 1-samenh.) | 3-sfeer $S^3$ (1-samenhangend) |
| **Isomorf met** | $U(1)$ | $SO(2)$ | — (dekgroep is $SU(2)$) | — ($SU(2) \to SO(3)$ is 2-op-1) |
| **Lie-algebra** | triviaal (1D, abels) | triviaal | $\mathfrak{so}(3)$ | $\mathfrak{su}(2) \cong \mathfrak{so}(3)$ |
| **Irreps** | $\varphi_n=e^{in\theta}$, $n\in\mathbb{Z}$ (1-dim, $\infty$ veel) | idem | $l=0,1,2,\dots$ (enkel integer!) | $l=0,\tfrac12,1,\tfrac32,\dots$ (alle) |
