...
\usepackage{amsmath,amssymb,amsfonts,textcomp, xfrac, siunitx, bm} % wiskunde shizzel
\usepackage{graphicx,wrapfig} % foto's
\usepackage{hyperref}
...
\begin{frame}{Introductie}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.55\linewidth]{../Figures-for-slides/potw1151a copy.png}
...
%Einstein schreef in 1936 dat we dit effect nooit zouden kunnen waarnemen, maar hij had daarbij enkel stellaire massa’s in gedachten. Het was onze oude bekende Fritz Zwicky die zich in 1937 realiseerde dat massieve clusters van sterrenstelsels we ́l in staat zijn om meerdere afbeeldingen van achterliggende sterrenstelsels te produceren.
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.5\linewidth]{../Figures-for-slides/grav-field-einstein.jpg}
...
\begin{frame}{Het Cosmic Horseshoe systeem}
    ...
    \begin{itemize}
        ...
        \item Hoe kunnen we de massa binnen een Einsteinring bepalen? 
        \item Wat zegt dit over een mogelijks centraal zwart gat?
        \item Kunnen we het gelensde sterrenstelsel reconstrueren?        \item Probleem: tijdsdilatatie van waarnemingen.
      \end{itemize}
\end{frame}
...
\begin{frame}{Luminous Red Galaxy}
    \begin{columns}[T] % [T] zorgt ervoor dat beide kolommen aan de bovenkant uitlijnen
        ...
        \begin{column}{0.6\textwidth}
            ...
            \vspace{0.2cm}
            De lens is een LRG in een groep of losse cluster in de quasar J1004+4112 met:
            \vspace{0.2cm}
            \begin{itemize}
            ...
            \vspace{0.2cm}
            De voornaamste bron is een stervormend sterrenstelsel van het type BX met roodverschuiving tussen $z = 2.379-2.381$.
        ...
        \begin{column}{0.38\textwidth}
            \begin{figure}
                \centering
                \includegraphics[width=0.8\linewidth]{Foto's/APOD-2025-FIG1.pdf}
                \caption{\tiny HST/WFC3-kleurencomplex van de Cosmic Horseshoe, samengesteld met de F814W-, F606W- en F475W-filters. Het systeem bestaat uit de hoofdafbuiger ($z_l = 0.44$), de Einsteinring van de Cosmic Horseshoe ($z_{\text{s2}} = 2.381$), en de radiale boog met zijn tegenbeeld ($z_{s1} = 1.961$), die beide zijn uitgelicht. De inzet toont de radiale boog. De figuur is zo georiënteerd dat het noorden bovenaan staat en het oosten links. 
                \copyright{C.~R. Melo-Carneiro et al. 2025. Published by Oxford University Press on behalf of Royal Astronomical Society.}}
            \end{figure}
        \end{column}
...
\begin{frame}{Data \& instrumenten I}
  \begin{itemize}
    \item Ontdekt in Sloan Digital Sky Survey (SDSS) (2007).  %hoge-res follow-up: HST (imaging), MUSE (kinematica).

    \item Waarnemingen: Wide Field Camera 3, 600s in de i, g en u banden.
  ...
  \end{itemize}
    ...
    \begin{columns}[t] % [t] zorgt dat de captions bovenaan uitlijnen als de tekst verschilt
        ...
        \begin{column}{0.48\textwidth}
            \begin{figure}
                \centering
                \includegraphics[width=0.8\linewidth]{Foto's/wfc3_inst-config.jpg}
        ...
        \begin{column}{0.48\textwidth}
            \begin{figure}
                \centering
                \includegraphics[width=0.8\linewidth]{Foto's/FIG1-2007.jpeg}
...
\begin{frame}{Modeleren}
    Hoe weten we welk model het best is?
    Evidence \(\epsilon\) functie om de verschillende modellen te rangschikken volgens de meest waarschijnlijke modellen.
    ...
    Met \(f_{ij}\) de flux in pixel \(j\) , \(d_j\) de waargenomen flux met \(\sigma_j\) zijn fout,  \(s_i\) is een component van een vector die de beste fit weergeeft voor de source pixel oppervlakte, \(\lambda\) zijn gewichten, \(\bm F\) en \(\bm H\) zijn regulariserende vierkante matrices.
    
    De beste modellen zijn te vinden door de evidence te maximaliseren.

    De standaardfouten gegeven door de diagonale termen van de covariante matrix:
    \[
...
\begin{itemize}
    ...
    \item Het identificeren van blauwe, lichtzwakke begeleiders (faint companions) rond (LRG's) in de SDSS cataloog.
...
\begin{frame}{Dynamische modellering I}
    ...
    De Jeans Anisotropic Modelling (JAM) benadering gaat ervan uit dat de massadistributie geparametriseerd kan worden als een som van concentrische elliptische Gaussen. 
    ...
    \begin{columns}[c]
        ...
        \begin{column}{0.5\textwidth}
            \begin{figure}
                ...
                \includegraphics[height=2.8cm, keepaspectratio]{Foto's/Fig7-2025.png}
...
\begin{frame}{Resultaten: }
    \begin{figure}
        \centering
        \includegraphics[width=0.5\linewidth]{Foto's/Fig2-2025.png}
...
We moeten rekening houden met drie componenten:
\begin{itemize}
...
\begin{frame}{Het massamodel II}
% Appendix B paper 2025
    ...
    \(\gamma^{\text{lens}}\) is de massa-dichtheid helling, dat tot een singular isothermal ellipsoid (SIE) reduceerd voor \(\gamma^{\text{lens}}=2\)
...
\begin{frame}{Het massa model III: Elliptische componenten}
    \begin{columns}[t] % [t] lijnt de kolommen aan de bovenkant uit
        ...
        \begin{column}{0.5\textwidth}
            ...
            We moeten ook rekening houden met de twee externe shears, waarvan de velden geparametriseerd zijn door twee elliptische componenten $(\epsilon_1^{sh}, \epsilon_2^{sh})$.
...
\begin{frame}{Multi-Gaussian Expansion I}
    De MGE methode paramtriseerd de stellaire oppervlaktehelderheid als een som van concentrische elliptische Gaussen met dezelfde oriëntatie.
    \[
...
\begin{frame}{Resultaten I: Reconstructie}
    ...
    \end{itemize}
            \begin{figure}
                \centering
                \includegraphics[width=\linewidth]{Foto's/Fig3-2025.png}
        ...
        \begin{column}{0.48\textwidth}
            \begin{figure}
                \centering
                \includegraphics[width=0.75\linewidth]{Foto's/Fig5-2025.png}
...
Het verschil tussen de Ly \(\alpha\) emissie en de interstellaire absorbtie roodverschuiving wordt typisch gevonden bij sterrenstelsels met een hoge SFR. De Ly \(\alpha\) lijn correspondeert normaal met 1216Å wordt hier verschoven naar \(\approx \)4100Å.
    \begin{figure}
        \centering
        \includegraphics[width=0.5\linewidth]{Foto's/FIG4-2007.jpeg}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{Foto's/fg5.h.jpg}
...
\begin{frame}{Resultaten II: \(\text{M}_{\text{BH}}-\sigma_e\) relatie}
    \begin{figure}
        \centering
        \includegraphics[width=0.42\linewidth]{Foto's/fig10-2025.png}
